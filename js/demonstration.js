...
$(document).ready(function() {
    ...
    $('label[for=normal]').click(function() {
        ...
        $('#param2-name').text('Standard deviation');
    ...
    $('#console-button').click(function() {
        if($('#console-dialog').is(':visible')) {
            $('#console-dialog').slideUp(200);
            $( "#console-button" ).button( "option", "icons", {primary:'ui-icon-gear',secondary:'ui-icon-triangle-1-n'} );
        } else {
            $( "#console-button" ).button( "option", "icons", {primary:'ui-icon-gear',secondary:'ui-icon-triangle-1-s'} );
    ...
    $('#prompt').keypress(function(event) {
        ...
        if(event.which == 13) {
                ...
                try {
                    ...
                    if(res != undefined && res != '' && command.indexOf('=') == -1) {
                        ...
                        if(res.constructor.toString().indexOf("Array") != -1) {
                            ...
                            $('#console-table tr:last').after('<tr><td><span class="data">' + array + '</span></td></tr>');
                        } else if(!isNaN(res)) {
            ...
            // stop IE from executing the example button
            return false;
        } else if(event.which == 38) {  // up arrow TODO: this doesnt work now (possible side effect from changing to keypress)
            if(index != -1) {
                if(index > 0) {
                    index--;
                }
    ...
    $('#execute-example').click(function() {
        with(jstat){
            var x = seq(0, 1, 100);
            var y = dbeta(x, 2, 2);
            plot(x,y,{
